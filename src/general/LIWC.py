...
def read_LIWC_dict(dict_path='../dataset/Spanish_LIWC2007_Dictionary.dic'):
    ...
    cat_gram = ['Yo', 'Nosotro', 'TuUtd', 'ElElla', 'VosUtds', 'Ellos', 'Pasado',
    ...
    cat_cog = ['MecCog', 'Insight', 'Causa', 'Discrep', 'Asentir', 'Tentat', 'Certeza', 'Inhib', 'Incl', 'Excl',
               'Percept', 'Ver', 'Oir', 'Sentir', 'NoFluen', 'Relleno', 'Ingerir', 'Relativ', 'Movim']
    cat_feels = ['Maldec', 'Afect', 'EmoPos', 'EmoNeg', 'Ansiedad', 'Enfado', 'Triste', 'Placer']
    cat_CDI = ['Articulo', 'Prepos', 'PronPer', 'PronImp', 'VerbAux', 'Conjunc', 'Adverb', 'Negacio']
    cat_location = []
    dic = {}
    ...
    cat_CDI = [split for split in splits if split[1] in cat_CDI]
    ...
    for word in dic:
        ...
        dic_CDI[word] = _assign_word_cat(word, cat_CDI, dic)
    dic_liwc = {'dic_gram': dic_gram, 'dic_obj': dic_obj, 'dic_cog': dic_cog, 'dic_feels': dic_feels, 'dic_CDI': dic_CDI}
    return dic_liwc
...
# if a word does not have a category, it is encoded as 'w'
def _assign_word_cat(word, macro_cat, dic):
    res = ''.join(cat[1] for n in dic[word] for cat in macro_cat if cat[0] == n)
    if res != '':
        return res
    else:
        return 'w'
